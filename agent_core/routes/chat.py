...
async def chat(request: ChatRequest, db: AsyncSession = Depends(get_db)):
    ...
    with _tracer.start_as_current_span("chat.request") as span:
        ...
        bundle = await build_messages(context, request.conversation_id, db, agent_config)
        response = await run_llm(bundle, context.tools, triggered_by=f"user:{request.user_id}")

        await conversation_manager.add_message(db, request.conversation_id, "assistant", response)
