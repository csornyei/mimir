...
import type { Message, ConversationSummary, LLMSettings, ServerEvent, ApprovalCard, ToolCall, ResponseMetadata } from '@/types'
...
const _approvalTimers = new Map<string, [ReturnType<typeof setTimeout>, ReturnType<typeof setTimeout>]>()
...
interface MimirStore {
  ...
  approveAction: (actionId: string, editedArgs?: Record<string, unknown>) => Promise<void>
...
function apiMessageToUiMessage(m: { id: number; role: string; content: string; timestamp: string }): Message {
  return {
    id: String(m.id),
    ...
    (set, get) => ({
      ...
      initWs: () => {
        ...
        ws.onopen = () => {
          ...
          if (_wasDisconnected) {
            toast.success('Reconnected to Mimir', { id: 'ws-disconnect' })
        ...
        ws.onmessage = (ev) => {
          try {
            ...
            console.error('[mimir] malformed WS frame:', err, ev.data)
          }
        }
        ...
        ws.onclose = () => {
          ...
          toast.warning('Disconnected from Mimir — reconnecting…', { id: 'ws-disconnect' })
      ...
      loadConversations: async () => {
        ...
        const conversations = data.map((c) => titles[c.id] ? { ...c, title: titles[c.id] } : c)
      ...
      loadConversation: async (id) => {
        set({ conversationLoading: true, messages: [], activeConversationId: id })
        try {
          ...
          const firstUser = uiMessages.find((m: Message) => m.role === 'user')
          const title = firstUser ? firstUser.content.substring(0, 60) : 'New conversation'
          set((s) => ({
            messages: uiMessages,
      ...
      deleteConversation: async (id) => {
        ...
        set((s) => {
          const { [id]: _removed, ...remainingTitles } = s.conversationTitles
          return {
      ...
      renameConversation: (id, title) => {
        set((s) => ({
          conversationTitles: { ...s.conversationTitles, [id]: title },
          conversations: s.conversations.map((c) => c.id === id ? { ...c, title } : c),
        }))
      },
      ...
      appendStreamEvent: (event) => {
        set((s) => {
          ...
          switch (event.type) {
            case 'thinking':
              msgs[lastIdx] = { ...last, thinkingContent: last.thinkingContent + event.content }
              return { messages: msgs }

            case 'response': {
              msgs[lastIdx] = { ...last, content: last.content + event.content }
              const updates: Partial<MimirStore> = { messages: msgs }
              // Capture conversation_id created by backend for new chats
              if (event.conversation_id && !s.activeConversationId) {
                updates.activeConversationId = event.conversation_id
              }
              return updates
            }

            case 'tool_call': {
              const tc: ToolCall = { call_id: event.call_id, name: event.name, arguments: event.arguments, result: null }
              msgs[lastIdx] = { ...last, toolCalls: [...last.toolCalls, tc] }
              return { messages: msgs }
            }
            ...
            case 'approval_required': {
              ...
              const { action_id: actionId, tool_name: toolName } = event
              const warn = setTimeout(() => {
                toast.warning(`Approval for "${toolName}" expires in 1 minute`)
              }, 9 * 60 * 1000)
              const expire = setTimeout(() => {
                toast.error(`Approval for "${toolName}" expired`)
                set((s) => ({
                  messages: s.messages.map((m) =>
                    m.approvalCard?.action_id === actionId
                      ? { ...m, approvalCard: { ...m.approvalCard!, status: 'rejected' as const } }
                      : m
                  ),
            ...
            case 'done':
              msgs[lastIdx] = { ...last, metadata: event.metadata as ResponseMetadata, isStreaming: false }
      ...
      setMode: (mode) => {
        set({ settings: MODE_PRESETS[mode], thinkingBudgetDirty: false })
      },

      setThinkingBudget: (budget) => {
        set((s) => ({ settings: { ...s.settings, thinking_budget: budget }, thinkingBudgetDirty: true }))
      },

      setEnableThinking: (enabled) => {
        set((s) => ({ settings: { ...s.settings, enable_thinking: enabled }, thinkingBudgetDirty: true }))
      },
      ...
      approveAction: async (actionId, editedArgs) => {
        ...
        const res = await fetch(`/api/approvals/${actionId}/approve`, {
          ...
          body: JSON.stringify(editedArgs ? { edited_arguments: editedArgs } : {}),
        })
        if (!res.ok) {
        ...
        set((s) => ({
          messages: s.messages.map((m) =>
            m.approvalCard?.action_id === actionId
              ? { ...m, approvalCard: { ...m.approvalCard!, status: 'approved' as const } }
              : m
          ),
          pendingApprovals: s.pendingApprovals.filter((id) => id !== actionId),
        }))
      ...
      rejectAction: async (actionId) => {
        const timers = _approvalTimers.get(actionId)
        if (timers) { clearTimeout(timers[0]); clearTimeout(timers[1]); _approvalTimers.delete(actionId) }
        const res = await fetch(`/api/approvals/${actionId}/reject`, { method: 'POST' })
        if (!res.ok) {
          const msg = await res.text().catch(() => 'Unknown error')
      ...
      setSelectedMessage: (id) => set({ selectedMessageId: id, rightPanelTab: id ? 'detail' : 'config' }),
