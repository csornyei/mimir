        ...
        <div className="px-6 py-4 max-w-3xl mx-auto">
          {loading ? (
            <div className="space-y-3">
              {[1, 2, 3].map((i) => (
                <Skeleton key={i} className="h-4" style={{ width: `${65 + i * 8}%` }} />
              ))}
            </div>
          ) : unavailable ? (
            <Alert className="border-border">
              <AlertDescription className="text-muted-foreground text-sm">
                Morning briefs are generated via Slack and are not available in the web UI.
              </AlertDescription>
            </Alert>
          ) : latest ? (
            ...
            <p className="text-sm text-muted-foreground text-center pt-8">No morning brief yet.</p>
          )}
        </div>
      </ScrollArea>
    </div>
