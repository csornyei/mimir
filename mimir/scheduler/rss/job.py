...
async def run_digest(
    ...
    if not slack_config.newspaper_channel_id:
        logger.warning(
    ...
    try:
        ...
        thread_ts = await post_digest_header(
            slack_config.newspaper_channel_id, len(entries), len(picks)
        )
        ...
        for pick in picks:
            ...
            message_ts = await post_pick(
                slack_config.newspaper_channel_id, thread_ts, pick
            )
            ...
            async with get_session() as session:
                session.add(
                    RssDigestEntry(
                        ...
                        digest_run_at=now,
                        window=window_label,
                        slack_channel_id=slack_config.newspaper_channel_id,
