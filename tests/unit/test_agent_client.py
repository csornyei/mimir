...
from agent_core.agent.client import send_to_agent
...
async def test_send_to_agent_returns_response_text(mocker):
    mocker.patch(
        "agent_core.agent.client.agent_config.agent_url", "http://localhost:8000"
    )
    ...
    with patch("agent_core.agent.client.httpx.AsyncClient", return_value=mock_client):
